...
public class WhitespaceTokenizer extends LeoBaseAnnotator {
	...
	@LeoConfigurationParameter(description = "The full path to a stop words file. Words in the stop word file are not annotated as word token if found in the document.")
	...
	public void initialize(UimaContext aContext) throws ResourceInitializationException {
		...
		String fileLocator = getStopWordsFile();
		...
		if (fileLocator != null)
		{
			...
			try {
				br = new BufferedReader(new InputStreamReader(getResourceAsInputStream(fileLocator)));
				String line = null;
	...
	public void annotate(JCas aJCas) throws AnalysisEngineProcessException {
		Feature tokenTypeFeature = aJCas.getTypeSystem().getType(getTokenOutputType())
				.getFeatureByBaseName(getTokenOutputTypeFeature());
	...
	private void createToken (JCas aJCas, int begin, int end, int tokenType, Feature tokenTypeFeature) throws AnalysisEngineProcessException {
		Annotation token = this.addOutputAnnotation(getTokenOutputType(), aJCas, begin, end);
		...
		if(tokenType == TK_WORD) {
			if(!filterStopWords) {
				this.addOutputAnnotation(getWordOutputType(), aJCas, begin, end);
			} else {
